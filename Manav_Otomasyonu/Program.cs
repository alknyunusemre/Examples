    ...
    internal class Program
    {
        ...
        static void Customer()
        {
            while (true)
            {
                ...
                Console.WriteLine("Meyve için 'M' Sebze İçin 'S' Tuşuna Basınız.");
                string frandvg = Console.ReadLine()!.ToUpper();
                if (sellervegetables.Count == 0 && Sellerfruit.Count == 0)
                {
                    Console.WriteLine("Henüz Satıcınızın Listesinde Ürün Bulunmamaktadır");
                    Console.WriteLine("Panele Geri Dönmek İçin Bir Tuşa Basınız");

                    Console.ReadKey();
                }
                if (frandvg == "M")
                {
                    for (int i = 0; i < Sellerfruit.Count; i++)
                    {
                        Console.WriteLine($"{Sellerfruit[i]} - {Sellerfruitprice[i]} TL\n{SellerProductCodeFruit[i]}");
                    }
                    Console.WriteLine("Almak İstediğiniz Ürünün Kodunu Giriniz:");
                    ...
                    {
                        for (int i = 0; i < sellervegetables.Count; i++)
                        {
                            Console.WriteLine($"{sellervegetables[i]} - {SellervegetablesPrice[i]} TL\n{SellerProductCodevegetables[i]}");
                        }
                        Console.WriteLine("Almak İstediğiniz Ürünün Kodunu Giriniz:");
                        ...
                        int productCode2;
                        if (int.TryParse(code2, out productCode2))
                        {
                        }
                        else
                        {
                            Console.WriteLine("Hatalı Tuşlama Yaptınız. Panele Yönlendiriliyorsunuz.");
                            Thread.Sleep(2000);
                            Console.Clear();
                        }
                    }
